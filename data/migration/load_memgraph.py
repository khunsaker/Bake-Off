#!/usr/bin/env python3
"""
Memgraph Data Loader for Shark Bake-Off
Loads generated JSON data into Memgraph graph database
Note: Memgraph uses same Cypher syntax as Neo4j but different port
"""
...
class MemgraphLoader:
    def __init__(self, uri, user="", password=""):
        # Memgraph doesn't require auth by default in docker
        if user:
    ...
    def create_constraints_and_indexes(self):
        """Create constraints and indexes (Memgraph syntax)"""
        print("\nCreating constraints and indexes...")

        with self.driver.session() as session:
            # Run the constraint/index creation script
            with open('../../schema/memgraph/001_constraints_indexes.cypher') as f:
        ...
        with self.driver.session() as session:
            ...
            query = """
                ...
                CREATE (o:Organization {
                    ...
                    country: org.country,
                    created_at: LocalDateTime(org.created_at)
                })
            """
        ...
        with self.driver.session() as session:
            query = """
                ...
                CREATE (l:Location {
                    ...
                    created_at: LocalDateTime(loc.created_at)
                })
            """
            ...
            for i in range(0, len(aircraft), batch_size):
                ...
                query = """
                    ...
                    CREATE (a:Aircraft {
                        ...
                        source: ac.source,
                        source_timestamp: LocalDateTime(ac.source_timestamp),
                        curator_modified: ac.curator_modified,
                        curator_id: ac.curator_id,
                        curator_locked: ac.curator_locked,
                        created_at: LocalDateTime(ac.created_at),
                        updated_at: LocalDateTime(ac.updated_at),
                        pg_id: ac.id
                    })
                """
        ...
        with self.driver.session() as session:
            query = """
                ...
                CREATE (s:Ship {
                    ...
                    source: ship.source,
                    source_timestamp: LocalDateTime(ship.source_timestamp),
                    curator_modified: ship.curator_modified,
                    curator_id: ship.curator_id,
                    curator_locked: ship.curator_locked,
                    created_at: LocalDateTime(ship.created_at),
                    updated_at: LocalDateTime(ship.updated_at),
                    pg_id: ship.id
                })
            """
            ...
            if seen_with:
                query = """
                    ...
                    CREATE (a1)-[:SEEN_WITH {
                        ...
                        first_seen: LocalDateTime(rel.properties.first_seen),
                        last_seen: LocalDateTime(rel.properties.last_seen),
            ...
            if visited:
                ...
                for i in range(0, len(visited), batch_size):
                    ...
                    query = """
                        ...
                        CREATE (s)-[:VISITED {
                            timestamp: LocalDateTime(rel.properties.timestamp),
                ...
                for i in range(0, len(refueled), batch_size):
                    ...
                    query = """
                        ...
                        CREATE (receiver)-[:REFUELED_BY {
                            ...
                            first_refuel: LocalDateTime(rel.properties.first_refuel),
                            last_refuel: LocalDateTime(rel.properties.last_refuel)
                        }]->(tanker)
                    """
                    session.run(query, rels=batch)
...
def main():
    ...
    try:
        loader = MemgraphLoader(uri, user, password)
        print("  ✓ Connected")
